...
from utils import get_file_type, format_file_size, read_config, save_config, get_uniqueid, create_config
...
class PackageExplorer:
    def __init__(self):
        ...
        self.osid = get_uniqueid()
        if exists(r".\settings.argon"):
            self.config = read_config(r".\settings.argon")
        else:
            self.config = create_config(r".\settings.argon")
    ...
    def create_popup_window(self, title, text, method):
        ...
        ok_button = tk.Button(self.popup_window, text="OK", command=method, width = 10)
        ...
        cancel_button.grid(row=1, column=2, padx=5, pady=5, sticky = "w")
    ...
    def create_preview_frame(self):
        ...
        text = "No Open Package" if self.ap.package is None else self.ap.package.split("/")[-1]
    ...
    def set_preview_title(self):
        if self.preview_title_label is not None:
            ...
            self.window.title(f"V Package Explorer | {text} | ..a project by VALKYTEQ")
    ...
    def open_file_explorer(self, targeted=False):
        ...
        subprocess.Popen(f'explorer "{vpk_folder}"')
    ...
    def get_application_info(self, item):
        ...
        try:
            ...
            app_size_formatted = format_file_size(app_size)
            app_mime = get_file_type(item[2])
    ...
    def get_image_info(self, item):
        ...
        try:
            ...
            image_size_formatted = format_file_size(image_size)
    ...
    def create_folder_tree(self):
        ...
        self.folder_tree = [[os.path.basename(self.ap.package), [], None]]
        for folder, file_dict in self.ap.byte_dict.items():
